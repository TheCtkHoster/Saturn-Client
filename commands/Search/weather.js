...
exports.run = async (client, message, args) => {
    ...
    await weather.find({search: city, degreeType: degreetype}, function(err, result) {
        ...
        .setDescription(`Pls type a city name..`)
        if (!city) return message.channel.send(well);
        const well1 = new Discord.MessageEmbed()
        .setDescription(`Unknown city, Pls try again..`)
        if (err || result === undefined || result.length === 0) return message.channel.send(well1);
